...
public class BurstLoadTaskGenerator {
    ...
    public List<TaskBean> generateTask() {
        List<TaskBean> taskConfigList = new ArrayList<>(numTasks);
        Random random = new Random();
        for (int i = 0; i < numTasks; ++i) {
            TaskBean burstLoadTaskConfig = new TaskBean();
            burstLoadTaskConfig.setExecTime(random.nextInt(taskExecTimeRange[1] - taskExecTimeRange[0]) + taskExecTimeRange[0]);
            burstLoadTaskConfig.setPacketSize(random.nextInt(taskPacketSizeRange[1] - taskPacketSizeRange[0]) + taskPacketSizeRange[0]);
